...
void genPolarCoord(double xOrigen,double yOrigen,double* vals,bool (*pred) (double,double)){
  double r = 0, theta = 0;
  do{
...
int main(int argc, char** argv){
  ...
  int i = 0;
  for(; i < n/2;i++)
      genPolarCoord(xOrigen,yOrigen,vals[i],pred1);
  showRes(n/2,2,vals,"datos1.txt");
  for(; i < n;i++)
      genPolarCoord(xOrigen,yOrigen,vals[i],pred2);
   //writeNames(2);
   //writeEntries(n,2,vals);
   showRes(n/2,2,vals+n/2,"datos2.txt");
