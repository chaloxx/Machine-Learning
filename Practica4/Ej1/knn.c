...
struct KNN {
    ...
    unsigned k; //Cantidad de vecinos cercanos que vamos a buscar
...
int output(struct KNN *knn,float *input,struct DATOS* datos){
  ...
  for(i=0; i<ctos; i++){
    index = datos->seq[i];
    distancias[i][0] = calcularDistanciaEuclidea(input,datos->data[index],knn->N_IN);
    distancias[i][1] = (float)datos->data[index][knn->N_IN];
  }

  //Ordenar de menor a mayor
  qsort(distancias,datos->PR,sizeof(float)*2,mayor);
  //Sumar votos
  for(i=0; i<knn->k;i++) class[(unsigned)distancias[i][1]] +=1 ;
  ...
  if(empate)
     return (unsigned)distancias[0][1];
...
int hacerEvaluaciones(struct KNN *knn,struct DATOS *datos){
  ...
  int error;
  int N_TOTAL;
  float train_error,valid_error,test_error,minValidError=1.0;
  FILE *fpredic;
  char filepat[100];
  unsigned bestK = 0; //Mejor valor para k q encontremos
 ...
 unsigned nroEvaluaciones = knn->k;
 for(i=1;i<=nroEvaluaciones;i++){
    knn->k=i;
    if(i == 1)train_error=evaluar(knn,datos,datos->data,0,datos->PR,1);
  ...
  /*mostrar errores*/
  printf("\nFin del entrenamiento: Mejor k es %d\n\n",bestK);
  printf("Errores:\nEntrenamiento:%f%%\n",train_error*100.);
