...
datos4 <-read.table("b30.mse",sep = "\t",header = FALSE)


n <- nrow(datos)
labels <- c("Batch:1","Batch:10","Batch:20","Batch:30")
colors <- c("Batch:1" = "blue","Batch:10" = "red", "Batch:20" = "green","Batch:30" = "yellow")
df <- data.frame(epoca=c(1:n,1:n,1:n,1:n),
                 errores=c(datos[,2],datos2[,2],datos3[,2],datos4[,2]),
                 class=c(rep(labels[1],n),rep(labels[2],n),rep(labels[3],n),rep(labels[4],n)) )
